...
async def run_expire_holds(
    ...
    async with sm() as session, session.begin():
        if booking_id is not None:
            ...
            stmt = (
                ...
                .where(Booking.status == "offered")
                .distinct()
            )
            res = await session.execute(stmt)
            target_resource_ids = list(res.scalars().all())
            await session.execute(
                text(
                    "UPDATE bookings "
                    "SET expires_at = clock_timestamp() - interval '5 seconds', "
    ...
    print(
        f"Hold expiry seam completed: {total_expired} hold(s) expired across {len(target_resource_ids)} resource(s)."
    )
...
def main() -> None:
    ...
    parser.add_argument(
        "--resource-id", type=uuid.UUID, default=None, help="Target resource UUID"
    )
    parser.add_argument(
        "--booking-id", type=uuid.UUID, default=None, help="Target offered booking UUID"
    )
    ...
    asyncio.run(run_expire_holds(args.resource_id, args.booking_id))
    sys.exit(0)
